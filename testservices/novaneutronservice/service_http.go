...
func (e *errorResponse) requestBody(r *http.Request) []byte {
	...
	body := e.body
	if body != "" {
		if e.novaNeutron != nil {
			body = strings.Replace(body, "$ENDPOINT$", e.novaNeutron.endpointURL(true, "/"), -1)
		}
...
type novaNeutronHandler struct {
	n      *NovaNeutron
	method func(n *NovaNeutron, w http.ResponseWriter, r *http.Request) error
}
...
func (n *NovaNeutron) handler(method func(n *NovaNeutron, w http.ResponseWriter, r *http.Request) error) http.Handler {
	return &novaNeutronHandler{n, method}
}
...
func (n *NovaNeutron) HandleRoot(w http.ResponseWriter, r *http.Request) {
	n.handler((*NovaNeutron).handleRoot).ServeHTTP(w, r)
}
	...
	case "GET":
		if suffix := path.Base(r.URL.Path); suffix != "servers" {
			...
			if groups {
				...
				if len(srvGroups) == 0 {
					srvGroups = []neutron.SecurityGroupV2{}
				}
				resp := struct {
...
func (n *NovaNeutron) SetupHTTP(mux *http.ServeMux) {
	...
	handlers = map[string]http.Handler{
		// "/":			n.handler((*NovaNeutron).handleApiVersions,
		"/v2.0/security-groups":       n.handler((*NovaNeutron).handleSecurityGroups),
		"/v2.0/security-groups/":      n.handler((*NovaNeutron).handleSecurityGroups),
		"/v2.0/security-group-rules":  n.handler((*NovaNeutron).handleSecurityGroupRules),
		"/v2.0/security-group-rules/": n.handler((*NovaNeutron).handleSecurityGroupRules),
		"/v2.0/floatingips":           n.handler((*NovaNeutron).handleFloatingIPs),
		"/v2.0/floatingips/":          n.handler((*NovaNeutron).handleFloatingIPs),
		"/v2.0/networks":              n.handler((*NovaNeutron).handleNetworks),
		"/v2.0/networks/":             n.handler((*NovaNeutron).handleNetworks),
		"/v2.0/subnets":               n.handler((*NovaNeutron).handleSubnets),
		"/v2.0/subnets/":              n.handler((*NovaNeutron).handleSubnets),
	}
	for path, h := range handlers {
		mux.Handle(path, h)
...
func (n *NovaNeutron) processGroupId(w http.ResponseWriter, r *http.Request) (*neutron.SecurityGroupV2, error) {
