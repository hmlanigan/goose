...
`,
	...
	errSecurityGroupProtocolRequiredWithPorts = &errorResponse{
		http.StatusNotFound,
		"",
		"Must also specify protocol if port range is given.",
		"",
		nil,
		nil,
	}
...
func (e *errorResponse) requestBody(r *http.Request) []byte {
	...
	if body != "" {
		if e.neutron != nil {
			body = strings.Replace(body, "$ENDPOINT$", e.neutron.endpointURL(false, "/"), -1)
		}
...
func (e *errorResponse) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	//fmt.Printf("e.ServeHTTP(): Method = %s; URL = %s\n", r.Method, r.URL)
	if e.contentType != "" {
...
func (h *neutronHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	//fmt.Printf("h.ServeHTTP(): Method = %s; URL = %s\n", r.Method, r.URL)
	path := r.URL.Path
...
func (n *Neutron) handleRoot(w http.ResponseWriter, r *http.Request) error {
	...
	//fmt.Printf("n.handleRoot() returning: errMultipleChoices\n")
