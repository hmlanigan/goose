...
func NewNoSwift(cred *identity.Credentials, authMode identity.AuthMode, useTLS bool) (*Openstack, []string) {
	...
	openstack.Neutron = neutronservice.New(openstack.URLs["neutron"], "v2", userInfo.TenantId, cred.Region, openstack.Identity, openstack.FallbackIdentity)
	return &openstack, logMsgs
}
...
func (openstack *Openstack) SetupHTTP(mux *http.ServeMux) {
	...
	openstack.Nova.SetupHTTP(openstack.muxes["neutron"])
